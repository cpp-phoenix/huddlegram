...
function Post() {
    ...
    const uploadFiles = async () => {
        if(videoData) {
            ...
            const url = ipfsLink + ".ipfs.w3s.link/" + videoData.name
            console.log("Url: ", url);
            try{
        ...
        <div className="flex flex-col items-center justify-center h-5/6 space-y-10">
            ...
            {videoData ? 
                <div className='flex flex-col space-y-4'>
                    ...
                    <button className='rounded-md border px-4 py-2 text-white bg-blue-500 hover:bg-blue-600' onClick={() => uploadFiles()}>Upload</button>
                </div> 
            : ""}
        </div>
    )
